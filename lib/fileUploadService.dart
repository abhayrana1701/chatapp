import 'dart:convert';
import 'dart:io';

import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:file_picker/file_picker.dart';
import 'package:firebase_storage/firebase_storage.dart';
import 'package:mychatapplication/databaseHelper.dart';
import 'package:mychatapplication/sendMessages.dart';
import 'package:uuid/uuid.dart';




class FileUploadService {
  // Static instance of FileUploadService to implement singleton pattern
  static final FileUploadService _instance = FileUploadService._internal();

  // Factory constructor to return the singleton instance
  factory FileUploadService() {
    return _instance;
  }

  // Private internal constructor
  FileUploadService._internal();

  // Firebase Storage instance for uploading files
  final FirebaseStorage _storage = FirebaseStorage.instance;

  // Map to store upload tasks associated with messageIds
  static final Map<String, UploadTask> _uploadTasksMap = {};

  // Pick multiple files using FilePicker
  Future<List<PlatformFile>?> pickFiles() async {
    FilePickerResult? result = await FilePicker.platform.pickFiles(
      allowMultiple: true,
    );
    if (result != null) {
      return result.files;
    } else {
      return null; // User canceled the picker
    }
  }

  // Upload files to Firebase Storage and store in sqflite
  Future<void> uploadFiles(
      List<PlatformFile> files, {
        required String currentUserId,
        required String receiverId,
        required String type,
        required String username,
        required Function(Map<String, dynamic> chatData) onSavedInLocalDb,
        required Function(String messageId, int isDelivered) onDeliveryStatusUpdated,
      }) async {
    final FirebaseFirestore _firestore = FirebaseFirestore.instance;
    DatabaseHelper databaseHelper = DatabaseHelper();

    List<String> messageIds = [];

    for (var file in files) {
      // Generate unique id for each message
      var uuid = Uuid();
      final messageId = uuid.v4();
      messageIds.add(messageId);

      // Create a map of the file details
      final fileDetails = {
        'name': file.name,
        'path': file.path,
        'size': file.size,
        'extension': type == 'file' ? file.extension : file.name.split('.').last,
      };

      // Convert the map to a JSON string
      String jsonString = jsonEncode(fileDetails);

      Map<String, dynamic> chatData = {
        'senderId': currentUserId,
        'messageId': messageId,
        'content': jsonString,
        'timestamp': DateTime.now().toIso8601String(),
        'messageType': type,
        'isRead': 0,
        'isReceived': 0,
        'isDelivered': -1,
      };

      // Insert chat into local database
      await databaseHelper.insertChat(currentUserId, receiverId, chatData);
      onSavedInLocalDb(chatData);
    }

    // Handle the uploads
    int i = 0;
    for (var file in files) {
      try {
        final String filePath = 'uploads/${file.name}';
        final fileBytes = await File(file.path!).readAsBytes();

        // Create the upload task
        UploadTask uploadTask = _storage.ref(filePath).putData(fileBytes);


        // Store the task in the map with messageId as the key
        String messageId = messageIds[i];
        _uploadTasksMap[messageId] = uploadTask;
        print("AAAAAAAAAAAAAAa $_uploadTasksMap");

        // Debugging: Log when task is added
        print("Upload task added for message ID: $messageId");

        uploadTask.whenComplete(() async {
          if (uploadTask.snapshot.state == TaskState.canceled) {
            print('Task was canceled. No further actions.');
            return;
          }
          String downloadURL = await _storage.ref(filePath).getDownloadURL();
          print('File uploaded: $downloadURL');

          // Store the reference in Firestore
          await _firestore.collection('chats').doc(SendMessages.getRoomId(currentUserId, receiverId))
              .collection('messages').add({
            'fileUrl': downloadURL.toString(),
            'senderId': currentUserId,
            'content': file.name,
            'messageId': messageId,
            'timestamp': FieldValue.serverTimestamp(),
            'messageType': type,
            'isRead': 0,
            'isReceived': 0,
            'isDelivered': 0,
            'receiverId': receiverId,
            'username': username,
            'size': file.size.toString(),
            'extension': type == 'file' ? file.extension : file.name.split('.').last,
          });

          databaseHelper.updateMessageDeliveryStatus(
            messageId: messageId,
            isDelivered: 1,
            userId1: currentUserId,
            userId2: receiverId,
          );

          try {
            onDeliveryStatusUpdated(messageId, 1);
          } catch (e) {}
        }).catchError((e) {
          // If upload fails
          String messageId = messageIds[i];
          onDeliveryStatusUpdated(messageId, 0);
          databaseHelper.updateMessageDeliveryStatus(
            messageId: messageId,
            isDelivered: 0,
            userId1: currentUserId,
            userId2: receiverId,
          );
          print('Error uploading file: $e');
        });

      } catch (e) {
        // If file upload fails
        String messageId = messageIds[i];
        onDeliveryStatusUpdated(messageId, 0);
        databaseHelper.updateMessageDeliveryStatus(
          messageId: messageId,
          isDelivered: 0,
          userId1: currentUserId,
          userId2: receiverId,
        );
        print('Error uploading file: ${file.name}, Error: $e');
      }

      i++; // Increment index after processing the file
    }
  }

  // Cancel an individual upload based on messageId
  void cancelUpload(String messageId) {
    if (_uploadTasksMap.containsKey(messageId)) {
      UploadTask? uploadTask = _uploadTasksMap[messageId];

      // Cancel the specific upload task
      uploadTask?.cancel();
      print('Upload canceled for message ID: $messageId');

      // Remove the task from the map
      _uploadTasksMap.remove(messageId);
    } else {
      print(_uploadTasksMap);
      print('No upload task found for message ID: $messageId');
    }
  }

  // Get current upload tasks (for debugging purposes)
  void getCurrentUploadTasks() {
    print('Current upload tasks: $_uploadTasksMap');
  }
}




// If anything went wrong then i will reuse this code


// class FileUploadService {
//   final FirebaseStorage _storage = FirebaseStorage.instance;
//
//   /// Pick multiple files using FilePicker
//   Future<List<PlatformFile>?> pickFiles() async {
//     FilePickerResult? result = await FilePicker.platform.pickFiles(
//       allowMultiple: true,
//     );
//     if (result != null) {
//       return result.files;
//     } else {
//       return null; // User canceled the picker
//     }
//   }
//
//   /// Upload files to Firebase Storage and store in sqflite
//   Future<void> uploadFiles(List<PlatformFile> files,
//       {required String currentUserId,
//         required String receiverId,
//         required String type,
//         required username,
//         required Function(Map<String, dynamic> chatData) onSavedInLocalDb,
//         required Function(String messageId,int isDelivered) onDeliveryStatusUpdated,
//       }) async {
//
//     final FirebaseFirestore _firestore = FirebaseFirestore.instance;
//
//     // Insert in database before uploading
//
//     List messageIds=[];
//     DatabaseHelper databaseHelper=DatabaseHelper();
//     for(var file in files){
//       //Generating unique id for each message
//       var uuid=Uuid();
//       final messageId=uuid.v4();
//       messageIds.add(messageId);
//       // Create a map of the file details
//       final fileDetails = {
//         'name': file.name,
//         'path': file.path,
//         'size': file.size,
//         'extension':type=='file'?file.extension:file.name.split('.').last,
//       };
//
//       // Convert the map to a JSON string
//       String jsonString = jsonEncode(fileDetails);
//
//       Map<String, dynamic> chatData={
//         'senderId':currentUserId,
//         'messageId':messageId,
//         'content':jsonString,
//         'timestamp':DateTime.now().toIso8601String(),
//         'messageType':type,
//         'isRead':0,
//         'isReceived':0,
//         'isDelivered':-1,
//       };
//
//       await databaseHelper.insertChat(
//           currentUserId,
//           receiverId,
//           chatData
//       );
//
//       onSavedInLocalDb(chatData);
//
//     }
//
//     int i=0;
//     var messageId;
//     for (var file in files) {
//       try {
//         // Upload each file to Firebase Storage
//         final String filePath = 'uploads/${file.name}';
//         // Read the file bytes
//         final fileBytes = await File(file.path!).readAsBytes();
//         UploadTask uploadTask = _storage.ref(filePath).putData(fileBytes);
//
//         // Continue uploading even when switching screens
//         uploadTask.whenComplete(() async {
//           // Get the download URL once the upload is complete
//           String downloadURL = await _storage.ref(filePath).getDownloadURL();
//           print('File uploaded: $downloadURL');
//
//
//           messageId=messageIds[i];
//           i++;
//           // Store the reference in Firestore
//           await _firestore.collection('chats').doc(SendMessages.getRoomId(currentUserId, receiverId))
//               .collection('messages').add({
//             'fileUrl': downloadURL,
//             'senderId':currentUserId,
//             'content': file.name,
//             'messageId':messageId,
//             'timestamp': FieldValue.serverTimestamp(),
//             'messageType': type,
//             'isRead':0,
//             'isReceived':0,
//             'isDelivered':0,
//             'receiverId':receiverId,
//             'username':username,
//             'size': file.size,
//             'extension':type=='file'?file.extension:file.name.split('.').last,
//           });
//
//           databaseHelper.updateMessageDeliveryStatus(messageId: messageId, isDelivered: 1, userId1: currentUserId, userId2: receiverId);
//           try{
//             print("yes ho gya");
//             onDeliveryStatusUpdated(messageId,1);
//           }catch(e){}
//
//         }).catchError((e) {
//           //If fail to deliver a message
//           onDeliveryStatusUpdated(messageId,0);
//           databaseHelper.updateMessageDeliveryStatus(messageId: messageId, isDelivered: 0, userId1: currentUserId, userId2: receiverId);
//           print('Error uploading file: $e');
//         });
//       } catch (e) {
//         //If fail to deliver a message
//         onDeliveryStatusUpdated(messageId,0);
//         databaseHelper.updateMessageDeliveryStatus(messageId: messageId, isDelivered: 0, userId1: currentUserId, userId2: receiverId);
//         print('Error uploading file: ${file.name}, Error: $e');
//       }
//     }
//   }
// }
